...
const response = await fetch(`${origin}/api/admin/brief-audio/${date}`, {
  method: "POST",
  headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
});
const payload = await response.json().catch(() => null);
if (!response.ok) throw new Error(`Unable to enqueue audio (HTTP ${response.status}, ${payload?.error?.code ?? "UNKNOWN"})`);
