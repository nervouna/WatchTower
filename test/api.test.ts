...
describe("public API", () => {
  beforeEach(async () => {
    await env.DB.exec("DELETE FROM brief_covers; DELETE FROM brief_audio; DELETE FROM item_sources; DELETE FROM brief_items; DELETE FROM briefs; DELETE FROM entities;");
  ...
  it("protects and idempotently queues the admin cover endpoint without public CORS", async () => {
    await replaceBrief(env.DB, draft());
    const unauthorized = await handleRequest(new Request("https://example.com/api/admin/brief-cover/2026-07-16", { method: "POST" }), env, now);
    expect(unauthorized.status).toBe(401);
    const request = () => new Request("https://example.com/api/admin/brief-cover/2026-07-16", { method: "POST", headers: { Authorization: "Bearer test-feedback-token" } });
    const queued = await handleRequest(request(), env, now);
    expect(await queued.json()).toMatchObject({ briefDate: "2026-07-16", status: "queued" });
    expect(queued.headers.get("Cache-Control")).toBe("no-store");
    expect(queued.headers.get("Access-Control-Allow-Origin")).toBeNull();
